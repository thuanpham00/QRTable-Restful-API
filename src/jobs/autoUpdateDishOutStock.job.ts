...
const autoUpdateStatusDishOutStockIngredient = async (fastify: FastifyInstance) => {
  // Chạy mỗi 5 phút
  try {
    // check từng món ăn coi các nguyên liệu của món này có đủ phục vụ không
    // rồi check tới từng menuItem liên quan để set status menuItem
    ...
    console.log(`[Cron] OutOfStock: ${outOfStockDishIds.length} dish(es), Restored: ${inStockDishIds.length} dish(es)`)
